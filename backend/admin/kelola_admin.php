...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($_GET['download']) && $_GET['download'] === 'true') {
        ...
        $fileName = "Admin.xlsx";
        // Header in Excel
        $column = array("NPK", "Nama Lengkap", "Jabatan", "Level");
        // Excel Data
        $sql = "SELECT a.npk, a.nama_lengkap, b.position, c.level
                FROM admin a LEFT JOIN position b ON a.id_position = b.id
                LEFT JOIN admin_level c ON a.id_level = c.id";
        ...
        if ($type == 'admin') {
            $result = $conn->query("SELECT a.id, a.npk, a.nama_lengkap, a.email, a.id_position, b.position, a.id_level, c.level, a.password
                                    FROM admin a LEFT JOIN position b ON a.id_position = b.id
                                    LEFT JOIN admin_level c ON a.id_level = c.id");
            if (!$result) {
        ...
        if ($type == 'admin_level') {
            $result = $conn->query("SELECT * FROM admin_level");
            if (!$result) {
                $response["msg"] = $conn->error;
                $response["status"] = "error";
                $conn->close();
    ...
    if($type == 'delete'){
        try{
            ...
            if($stmt->execute()){
                ...
                $response["msg"] = "Admin has been successfully deleted!";
            }else{
                $response["status"] = "error";
    ...
    if($type == 'edit' || $type == 'add'){
        ...
        $idLevel = cleaner($_POST["level"], $conn);
        $id = cleaner($_POST["id"], $conn);
        ...
        if (!empty($npk)) {
            $getNpk = $conn->prepare("SELECT npk FROM admin WHERE npk = ? AND id != ?");
        ...
        if (!empty($email)) {
            $getEmail = $conn->prepare("SELECT email FROM admin WHERE email = ? AND id != ?");
        ...
        if($type == 'edit'){
            try{
                // update admin
                $stmt = $conn->prepare("UPDATE admin SET npk=?, nama_lengkap=?, email=?, id_position=?, id_level=? WHERE id=?");
                $stmt->bind_param("issiii", $npk, $name, $email, $idPosition, $idLevel, $id);
                if($stmt->execute()){
                    if ($_SESSION["data"]["npk"] == $id) {
                        $_SESSION["data"]["npk"] = $npk;
                        $_SESSION["data"]["namaLengkap"] = $name;
                    }

                    $response["status"] = "success";
                    $response["title"] = "Edit Success!";
                    $response["msg"] = "Admin data has been successfully changed!";
                }else{
                    $response["status"] = "error";
            ...
            try{
                // Insert to admin
                $stmt = $conn->prepare("INSERT INTO admin (npk, nama_lengkap, email, password, id_position, id_level) 
                                        VALUES(?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("isssii", $npk, $name, $email, $hashed_password, $idPosition, $idLevel);
                if($stmt->execute()){
                    $response["status"] = "success";
                    $response["title"] = "Registration Successful!";
                    $response["msg"] = "Admin has been successfully added!";
                }else{
                    $response["status"] = "error";
